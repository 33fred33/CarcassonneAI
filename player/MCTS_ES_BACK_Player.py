...
class MCTS_ES_BACK_Player(Player):
    
    # MCTS with ES and backpropogation
    
    def __init__(self, iterations = 500, timeLimit = 10, isTimeLimited = False, c_param = 3, logs=False, logfile=None, name='ES_B_MCTS', Lambda=4, NGen=20, ES_Sims=30, ESType = "plus"):
        super().__init__()
    ...
    def ClonePlayer(self):
        Clone = MCTS_ES_BACK_Player(iterations=self.iterations, timeLimit=self.timeLimit, isTimeLimited = self.isTimeLimited, c_param=self.c_param, 
                                    logs=self.logs, logfile=self.logfile, name=self.name, Lambda=self.Lambda, NGen=self.NGen, ES_Sims=self.ES_Sims, ESType = self.ESType)
        return Clone
...
def selBestCustom(individuals, MCTS_Player, generation, turn, fit_attr="fitness"):
    ...
    for i in individuals:
        ...
        SSD += round(semanticsDistance(individuals[0], i), 3) # SSD between each new individual and sole parent
        TotalDepth += i.height
