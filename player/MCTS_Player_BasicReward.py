...
class MCTSPlayer_BasicReward(Player):
    
    # Player 1 selects the optimal UCT move 
    # Player 2 selects the worst move from Player 1's position
    
    def __init__(self, iterations = 500, timeLimit = 10, isTimeLimited = False, c_param = 1, logs=False, logfile=None, name='MCTS_BasicReward'):
        super().__init__()
    ...
    def ClonePlayer(self):
        Clone = MCTSPlayer_BasicReward(iterations=self.iterations, timeLimit=self.timeLimit, isTimeLimited = self.isTimeLimited, 
...
class Node:
    ...
    def UpdateNode(self, result, c_param):
        ...
        sign = 1
        if result < 0: sign = -1
        reward = 0 if (result == 0) else (sign)
        
        self.Q = self.Q + (reward - self.Q)/self.visits
