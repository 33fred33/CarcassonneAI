...
class MCTS_ES_Player(Player):
    ...
    def __init__(self, iterations = 500, timeLimit = 10, isTimeLimited = False, c_param = 3, logs=False, logfile=None, name='ES_MCTS', Lambda=4, NGen=20, ES_Sims=30, ESType = "plus"):
        super().__init__()
    ...
    def ClonePlayer(self):
        Clone = MCTS_ES_Player(iterations=self.iterations, timeLimit=self.timeLimit, isTimeLimited = self.isTimeLimited, c_param=self.c_param, 
                                    logs=self.logs, logfile=self.logfile, name=self.name, Lambda=self.Lambda, NGen=self.NGen, ES_Sims=self.ES_Sims)
        return Clone
...
def ES_Search(RootNode, MCTS_Player):
    ...
    def evalTree(individual, RootNode, state):
        ...
        nodeValues = [[c.Q, c.visits, RootNode.visits] for c in childNodes]
